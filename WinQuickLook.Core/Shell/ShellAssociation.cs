...
internal class ShellAssociation
{
    public void GetRecommends(FileInfo fileInfo)
    {
    ...
    {
        Span<char> iconImageLocation = new char[512];

        if (PInvoke.SHLoadIndirectString(path, iconImageLocation).Failed)
        {
            return null;
        }
